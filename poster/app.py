...
class Poster(tweeter_pb2_grpc.PosterServicer):
    ...
    def PostMessageWithImage(self, request, context):
        logging.info("Request received to post a message with an image")

    def PostMessageWithVideo(self, request, context):
        logging.info("Request received to post a message with Videos")


def post_message(request: pb.PostMessageRequest()) -> pb.PostMessageResponse():
